...
import com.naruku.sax.content.AttributeName;
import com.naruku.sax.content.CellDataType;
import com.naruku.sax.content.ElementName;
...
public class SheetDataSaxHandler extends DefaultHandler {
	...
	private void endCell() {
		...
		final String contentStr = lastContent.toString().trim().replaceAll("\r|\n", "").replace("\n","");
		...
		if (lastFormula.length() == 0) {
			value = new FormulaCellValue(lastFormula.toString().trim(), value);
		}
	...
	private void setCellType(Attributes attributes) {
		...
		if (null != this.stylesTable) {
			...
			if (null != xfIndexStr) {
				...
				if (xssfCellStyle.getDataFormatString() == null) {
					this.numFmtString = BuiltinFormats.getBuiltinFormat(numFmtIndex);
				} else {
